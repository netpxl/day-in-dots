...
import { lastValueFrom, Observable, of, ReplaySubject, Subject, take } from 'rxjs';
...
export class LocalStorageStoreService extends StoreService {
  ...
  mergeWithActivites(config: DayBoardInterface, activities: ActivityInterface[]) {
    config.board.forEach((row) => {
      row.map((dot, index) => {
        const activity = activities.find((activity) => activity.id === dot.activityId)
        if (!activity) {
          delete dot.color;
        ...
        row[index] = {...dot, color: activity.color}
      })
    })
    return config;
  }
